...
/*
void	put_snake(t_data *app, int x, int y)
{
	int					x_body;
	int					y_body;
	t_snake				*head;
	t_list 				*body;
	t_map_fesg_addr		*head_addr;
	t_map_fesg_addr		*body_addr;

	head = (&app->game->s);
	head->xh_pos = (x * BLOCK) + 42;
	head->yh_pos = (y * BLOCK) + 42;
	head_addr = (&app->game->spr.fesg.snake_head);
	assign_image(app, (&head->head), head_addr);
	body = app->game->s.body;
	body_addr = (&app->game->spr.fesg.snake_body);
	body = 
	x_body = (x * BLOCK) + 84;
	y_body = (x * BLOCK) + 84;
